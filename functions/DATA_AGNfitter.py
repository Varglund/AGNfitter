    ...
    def PROPS(self):
        if self.cat["filetype"] == "ASCII":
            ...
            if self.cat["use_central_wavelength"]:
                ...
                names = np.loadtxt(
                    self.cat["path"] + "models/FILTERS/ALL_FILTERS_info.dat",
                    delimiter="|",
                    usecols=[1],
                    skiprows=1,
                    dtype=str,
                )
                centralwls = np.loadtxt(
                    self.cat["path"] + "models/FILTERS/ALL_FILTERS_info.dat",
                    delimiter="|",
                    usecols=[3],
                    skiprows=1,
                ...
                for i in range(len(list(dictionary.keys()))):
                    for j in range(len(names)):
                        try:
                            ...
                            if (
                                list(dictionary.keys())[i] == names[j]
                                and dictionary[list(dictionary.keys())[i]][0]
                            ):
                                list_centralwls.append(
                                    [
                ...
                freq_wl_cat_ALL = (
                    np.array([column.iloc[:, c] for c in self.cat["freq/wl_list"]])
                    * self.cat["freq/wl_unit"]
                )

            flux_cat_ALL = (
                np.array(column.iloc[:, self.cat["flux_list"]]).astype(float)
                * self.cat["flux_unit"]
            )
            fluxerr_cat_ALL = (
                np.array(column.iloc[:, self.cat["fluxerr_list"]]).astype(float)
                * self.cat["flux_unit"]
            )
            ...
            for j in range(nrSOURCES):
                ...
                if self.cat["use_central_wavelength"]:
                    ...
                    if self.cat["freq/wl_format"] == "wavelength":
                        nus0 = np.log10(
                            freq_wl_cat.to(u.Hz, equivalencies=u.spectral()).value
                        )

                fluxes0 = np.array(flux_cat.to(u.erg / u.s / (u.cm) ** 2 / u.Hz).value)
                fluxerrs0 = np.array(
                    fluxerr_cat.to(u.erg / u.s / (u.cm) ** 2 / u.Hz).value
                )
                ...
                if self.cat["ndflag_bool"] == True:
                    ...
                    ndflag_cat0[fluxerr_cat.value / units_flags <= -99] = 0.0
                    fluxes0[
                        (fluxerr_cat.value / units_flags <= -99)
                        & (flux_cat.value / units_flags > -99)
                    ] = (
                        fluxes0[
                            (fluxerr_cat.value / units_flags <= -99)
                            & (flux_cat.value / units_flags > -99)
                ...
                if self.cat["err+10%flux_moreflex"] == True:
                    ...
                    fluxerrs0[ndflag_cat0 != 0] = np.sqrt(
                        fluxerrs0[ndflag_cat0 != 0] ** 2
                        + (fluxes0[ndflag_cat0 != 0] * 0.1) ** 2
                    )
                ...
                RADdata_pos = nus0[nus0.argsort()] < (
                    10.5 - np.log10(1 + self.z[j])
                )  # < 30 GHz rest frame
                RADdata = fluxes0[nus0.argsort()][
                    (RADdata_pos == True) & (ndflag_cat0[nus0.argsort()] > 0)
                ]
                ...
                XRdata_pos = nus0[nus0.argsort()] > (
                    16.685 - np.log10(1 + self.z[j])
                )  # > 0.2 keV rest frame
                XRdata = fluxes0[nus0.argsort()][
                    (XRdata_pos == True) & (ndflag_cat0[nus0.argsort()] > 0)
                ]
                ...
                for i in range(len(dictionary.keys())):
                    for j in range(len(names)):
                        try:
                            ### The filter dictionary need to have to entries [True/False, column_number]
                            if (
                                dictionary.keys()[i] == names[j]
                                and dictionary[dictionary.keys()[i]][0]
                            ):
                                list_centralwls.append(
                                    [dictionary[dictionary.keys()[i]][1], centralwls[j]]
                                )
                        except:
            ...
            flux_cat_ALL = (
                np.array([fitstable[ca] for ca in flux_cols]).astype(float)
                * self.cat["flux_unit"]
            )
            fluxerr_cat_ALL = (
                np.array([fitstable[ce] for ce in flux_err_cols]).astype(float)
                * self.cat["flux_unit"]
            )
            if self.cat["ndflag_bool"] == True:
            ...
            for j in range(nrSOURCES):
                ...
                if self.cat["ndflag_bool"] == True:
                    ...
                    # implying an uncertanty that ranges from [0,uppflux]
                    units_flags = self.cat["flux_unit"].value
                    ndflag_cat0[flux_cat.value / units_flags <= -99] = 0.0
                    fluxes0[
                        (fluxerr_cat.value / units_flags <= -99)
                        & (flux_cat.value / units_flags > -99)
                    ] = (
...
class DATA:
    ...
    def __init__(self, data_all, line):
        ...
        self.lumfactor = 4.0 * pi * catalog.dlum[line] ** 2.0
