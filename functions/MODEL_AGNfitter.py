...
from astropy.cosmology import (
    FlatLambdaCDM,
)  # Cosmology that we assume for estimate luminosity distance
...
def GALAXYfunctions():
    def apply_reddening(gal_nu, gal_Fnu, EBV_gal):
        gal_nu, gal_Fnu_red = GALAXYred_Calzetti(
            gal_nu, gal_Fnu.flatten(), float(EBV_gal)
        )
...
def GALAXY(path, modelsettings):
    ...
    if modelsettings["GALAXY"] == "BC03":
        ...
        BC03dict = pd.read_pickle(
            open(path + "models/GALAXY/BC03_840seds.pickle", "rb")
        )
        ...
        if parameters_types[2] == "grid":
            ebvgal_array = np.array(np.arange(0.0, 100.0, 2.5) / 100)
            ...
            for c in par_idxs_combinations:
                ...
                gal_wl, gal_Fwl = (
                    BC03dict["wavelength"],
                    BC03dict["SED"][:, agei, taui, :, :, :].squeeze(),
                )
                gal_nus = gal_wl.to(u.Hz, equivalencies=u.spectral())[::-1]  # invert
                gal_Fnu = (gal_Fwl * 3.34e-19 * gal_wl**2.0)[::-1]  # Fnu2Flambda
                gal_SFR = BC03dict["SFR"][:, agei, taui, :, :].squeeze()
                gal_nu, gal_Fnu_red = GALAXY_functions[0](
                    gal_nus.value[0 : len(gal_nus) : 3],
                    gal_Fnu.value[0 : len(gal_nus) : 3],
                    ebvgal_array[ebvi],
                )  # erg/s/Hz
                GALAXYFdict_4plot[
                    str(tau_array.value[taui]),
                    str(np.log10(age_array.value[agei])),
                    str(ebvgal_array[ebvi]),
                ] = np.log10(gal_nu), renorm_template("GA", gal_Fnu_red)
                GALAXY_SFRdict[
                    str(tau_array.value[taui]), str(np.log10(age_array.value[agei]))
                ] = gal_SFR
                gal_Fnu_int = scipy.integrate.trapz(
                    gal_Fnu.value[0 : len(gal_nus) : 3],
                    x=gal_nus.value[0 : len(gal_nus) : 3],
                )
        ...
        BC03dict = pd.read_pickle(
            open(path + "models/GALAXY/BC03_seds_metal_medium.pickle", "rb")
        )
        ...
        if parameters_types[3] == "grid":
            ebvgal_array = np.array(np.arange(0.0, 60.0, 5.0) / 100)
            ...
            for c in par_idxs_combinations:
                ...
                gal_wl, gal_Fwl = (
                    BC03dict["wavelength"],
                    BC03dict["SED"][metali, agei, taui, :, :, :].squeeze(),
                )
                gal_nus = gal_wl.to(u.Hz, equivalencies=u.spectral())[::-1]  # invert
                gal_Fnu = (gal_Fwl.value * 3.34e-19 * gal_wl**2.0)[::-1]
                gal_nu, gal_Fnu_red = GALAXY_functions[0](
                    gal_nus.value[0 : len(gal_nus) : 3],
                    gal_Fnu.value[0 : len(gal_nus) : 3],
                    ebvgal_array[ebvi],
                ...
                gal_Fnu_int = scipy.integrate.trapz(
                    gal_Fnu[0 : len(gal_nus) : 3] * 3.826e33, x=gal_nu
                )
        ...
        BC03dict = pd.read_pickle(
            open(path + "models/GALAXY/BC03_metal_rxLARGE.pickle", "rb")
        )
...
def STARBURST(path, modelsettings):
    ...
    if modelsettings["STARBURST"] == "DH02_CE01":
        ...
        DH02CE01dict = pd.read_pickle(
            open(path + "models/STARBURST/DH02_CE01.pickle", "rb")
        )
        ...
        for irlumi in range(irlumidx):
            sb_nu0, sb_Fnu0 = (
                DH02CE01dict["wavelength"][irlumi],
                DH02CE01dict["SED"][irlumi].squeeze(),
            )
            STARBURSTFdict_4plot[str(DH02CE01dict["irlum-values"][irlumi])] = (
                sb_nu0,
                renorm_template("SB", sb_Fnu0),
            )
            STARBURST_LIRdict[str(DH02CE01dict["irlum-values"][irlumi])] = (
                pow(10, DH02CE01dict["irlum-values"][irlumi]) * 3.826e33 * 1e-6
            )
        ...
        modelsettings["STARBURST"] == "S17"
    ):  # Model created specifically for AGNfitter, corrected by very small grains (VSG correction)
        STARBURSTFdict_4plot = dict()
        STARBURST_LIRdict = dict()
        # Call object containing all starburst models
        ...
        Lir = (
            np.array(dusttable["LIR"])[0] * 3.826e33
        )  ###!!!*1e-6 #Lsun2ergs ### consider taking away renormalizaion 1e-6
        fracPAH = np.concatenate(
        ...
        for c in par_idxs_combinations:
            ...
            sb_Fnu0 = np.array(
                (1 - fracPAH[fp]) * DLnu[t, :] + (fracPAH[fp]) * PLnu[t, :]
            )[::-1]

            STARBURSTFdict_4plot[str(Tdust[t]), str(fracPAH[fp])] = (
                np.log10(sb_nu0),
                renorm_template("SB", sb_Fnu0),
            )
        ...
        modelsettings["STARBURST"] == "S17_radio"
    ):  # S17_newmodel + the infrared-radio correlation by Bell (2003) for the host galaxy radio emission
        STARBURSTFdict_4plot = dict()
        STARBURST_LIRdict = dict()

        # Call object containing all starburst models
        dusttable = Table.read(
            path + "models/STARBURST/s17_lowvsg_dust+radio+sigma.fits"
        )  # Frequencies are in increasing order
        pahstable = Table.read(
            path + "models/STARBURST/s17_lowvsg_pah.fits"
        )  # Wavelengths are in increasing order, it's necessary to convert
        ...
        for c in par_idxs_combinations:
            ...
            sb_Fnu0 = np.array(
                (1 - fracPAH[fp]) * DLnu[t, :]
                + (fracPAH[fp]) * np.concatenate((np.zeros((23)), PLnu[t, :][::-1]))
            )
...
def AGN_RAD(path, modelsettings, nRADdata):
    ...
    if modelsettings["RADIO"] == True:
        ...
        if nRADdata == 1:  # 1
            ...
            AGN_RADFdict_4plot["-99.9"] = (
                np.log10(agnrad_nu),
                renorm_template("AGN_RAD", agnrad_Fnu),
            )
            ...
            if modelsettings["Blazar"] == "SPL":
                ...
                for c in par_idxs_combinations:
                    ...
                    agnrad_Fnu = ((agnrad_nu / nu_t) ** alpha_i) * np.exp(
                        -agnrad_nu / (10**E_cutoffi)
                    )
                    AGN_RADFdict_4plot[str(alpha_i), str(E_cutoffi)] = (
                        np.log10(agnrad_nu),
                        renorm_template("AGN_RAD", agnrad_Fnu),
                    )
            ...
            for c in par_idxs_combinations:
                ...
                agnrad_Fnu = (
                    ((agnrad_nu / 10**nu_ti) ** alpha1i)
                    * (1 - np.exp(-((10**nu_ti / agnrad_nu) ** (curvi))))
                    * np.exp(-agnrad_nu / (10**E_cutoffi))
                )
                if modelsettings["Blazar"] == "DPL-3":
                    AGN_RADFdict_4plot[str(curvi), str(nu_ti), str(E_cutoffi)] = (
                        np.log10(agnrad_nu),
                        renorm_template("AGN_RAD", agnrad_Fnu),
                    )
                else:
                    AGN_RADFdict_4plot[str(curvi), str(nu_ti)] = (
                        np.log10(agnrad_nu),
                        renorm_template("AGN_RAD", agnrad_Fnu),
                    )

        elif (
            nRADdata > 3 or modelsettings["Blazar"] == "DPL-4"
        ):  # Double power law with cutoff with parameters
            ...
            for c in par_idxs_combinations:
                ...
                agnrad_Fnu = (
                    ((agnrad_nu / 10**nu_ti) ** alpha1i)
                    * (1 - np.exp(-((10**nu_ti / agnrad_nu) ** (alpha1i - alpha2i))))
                    * np.exp(-agnrad_nu / (10**E_cutoffi))
                )
                if modelsettings["Blazar"] == "DPL-4":
                    AGN_RADFdict_4plot[
                        str(alpha1i), str(alpha2i), str(nu_ti), str(E_cutoffi)
                    ] = np.log10(agnrad_nu), renorm_template("AGN_RAD", agnrad_Fnu)
                else:
                    AGN_RADFdict_4plot[str(alpha1i), str(alpha2i), str(nu_ti)] = (
                        np.log10(agnrad_nu),
                        renorm_template("AGN_RAD", agnrad_Fnu),
                    )
...
def BBBfunctions():
    ...
    def add_xrays(bbb_nu, bbb_Fnu, EBV_bbb, alpha_scat, gamma=1.8):
        bbb_nu0, bbb_Fnu_red = BBBred_Prevot(bbb_nu, bbb_Fnu.flatten(), float(EBV_bbb))
        xray_nu, xray_Fnu = XRAYS(
            bbb_nu, bbb_Fnu.flatten(), float(alpha_scat), float(gamma)
        )
        # R06 SED is extended and we need avoid overlapping between BBB template and X-Rays power-law
        bbb_nu0x, bbb_Fnu_redx = (
            np.concatenate((bbb_nu0[bbb_nu0 < 16.685], xray_nu)),
            np.concatenate((bbb_Fnu_red[bbb_nu0 < 16.685], xray_Fnu)),
        )
...
def BBB(path, modelsettings, nXRaysdata):
    ...
    if modelsettings["BBB"] == "R06":
        ...
        if (
            modelsettings["XRAYS"] != True
        ):  # If there no x-rays data or priors will be apply, don't use the UV-Xrays correlation to extend SEDs
            parameters_names = ["EBVbbb"]
            parameters_types = ["free"]

            if parameters_types == ["grid"]:
                ebvbbb_array = np.array(np.arange(0.0, 100.0, 5.0) / 100)
                for EBV_bbb in ebvbbb_array:
                    # Apply reddening
                    bbb_nu0, bbb_Fnu_red = BBB_functions[0](bbb_nu, bbb_Fnu, EBV_bbb)
                    # Construct dictionaries
                    BBBFdict_4plot[str(EBV_bbb)] = (
                        bbb_nu0,
                        renorm_template("BB", bbb_Fnu_red),
                    )
            elif parameters_types == ["free"]:
                ebvbbb_array = np.array(
                    [0.0, 1.0]
                )  # write limits for the free parameter
                for EBV_bbb in ebvbbb_array:
                    # Apply reddening. Using free parameters the templates must be saved without the effect of that/those parameter/s.
                    bbb_nu0, bbb_Fnu_red = BBB_functions[0](
                        bbb_nu, bbb_Fnu, ebvbbb_array[0]
                    )
                    # Construct dictionaries
                    BBBFdict_4plot[str(EBV_bbb)] = (
                        bbb_nu0,
            ...
            modelsettings["XRAYS"] == True and nXRaysdata <= 1
        ):  # Use the UV-Xrays correlation (Just et al. 2007) to extend SEDs
            ...
            if parameters_types == ["grid", "grid"]:
                ...
                for c in par_idxs_combinations:
                    ...
                    bbb_nu0x, bbb_Fnu_redx = BBB_functions[1](
                        bbb_nu, bbb_Fnu, ebvi, alpha_scati
                    )
                    # Construct dictionaries
                    BBBFdict_4plot[str(ebvi), str(alpha_scati)] = (
                        bbb_nu0x,
                        renorm_template("BB", bbb_Fnu_redx),
                    )
            elif parameters_types == ["free", "free"]:
                ebvbbb_array = np.array([0.0, 1.0])
                ...
                for c in par_idxs_combinations:
                    ...
                    bbb_nu0x, bbb_Fnu_redx = BBB_functions[1](
                        bbb_nu, bbb_Fnu, ebvbbb_array[0], 0
                    )
                    # Construct dictionaries
                    BBBFdict_4plot[str(ebvi), str(alpha_scati)] = (
                        bbb_nu0x,
            ...
            if parameters_types == ["grid", "grid", "grid"]:
                ...
                for c in par_idxs_combinations:
                    ...
                    # Apply reddening + UV-Xrays correlation
                    bbb_nu0x, bbb_Fnu_redx = BBB_functions[1](
                        bbb_nu, bbb_Fnu, ebvi, alpha_scati, gammai
                    )
                    # Construct dictionaries
                    BBBFdict_4plot[str(ebvi), str(alpha_scati), str(gammai)] = (
                        bbb_nu0x,
                        renorm_template("BB", bbb_Fnu_redx),
                    )
                ...
                for c in par_idxs_combinations:
                    ...
                    # Apply reddening + UV-Xrays correlation. Using free parameters the templates must be saved without the effect of that/those parameter/s.
                    bbb_nu0x, bbb_Fnu_redx = BBB_functions[1](
                        bbb_nu, bbb_Fnu, ebvbbb_array[0], 0, gammai
                    )
                    # Construct dictionaries
                    BBBFdict_4plot[str(ebvi), str(alpha_scati), str(gammai)] = (
                        bbb_nu0x,
    ...
    ## Model from Slone and Netzer 2012
        ...
        if (
            ...
            for c in par_idxs_combinations:
                ...
                EddRi = c[1]
                ebvi = c[2]
                bbb_nu, bbb_Fnu_nored = (
                    np.log10(SN12dict["frequency"]),
                    SN12dict["SED"][:, Mbhi, EddRi].squeeze(),
                )
                if parameters_types[2] == "grid":
                    # Apply reddening
                    bbb_nu0, bbb_Fnu_red = BBB_functions[0](
                        bbb_nu, bbb_Fnu_nored, ebvbbb_array[ebvi]
                    )
                elif parameters_types[2] == "free":
                    # Apply reddening. Using free parameters the templates must be saved without the effect of that/those parameter/s.
                    bbb_nu0, bbb_Fnu_red = BBB_functions[0](
                        bbb_nu, bbb_Fnu_nored, ebvbbb_array[0]
                    )
                # Construct dictionaries
                BBBFdict_4plot[
                    str(Mbh_array[Mbhi]),
                    str(EddR_array[EddRi]),
                    str(ebvbbb_array[ebvi]),
                ] = bbb_nu0, bbb_Fnu_red
            ...
            modelsettings["XRAYS"] == True and nXRaysdata <= 1
        ):  # Use the UV-Xrays correlation (Just et a. 2007) to extend SEDs
            ...
            if parameters_types[2:4] == ["grid", "grid"]:
                ...
                idxs = [
                    np.arange(Mbhidx),
                    np.arange(EddRidx),
                    np.arange(len(ebvbbb_array)),
                    np.arange(len(alpha_scat)),
                ]
                par_idxs_combinations = np.array(list(itertools.product(*idxs)))
                for c in par_idxs_combinations:
                    Mbhi = c[0]
                ...
                for c in par_idxs_combinations:
                    ...
                    # Apply reddening + UV-Xrays correlation. Using free parameters the templates must be saved without the effect of that/those parameter/s.
                    bbb_nu0x, bbb_Fnu_redx = BBB_functions[1](
                        bbb_nu, bbb_Fnu_nored, ebvbbb_array[0], 0
                    )
                    # Construct dictionaries
                    BBBFdict_4plot[
                        str(Mbh_array[Mbhi]),
            ...
            if parameters_types[2:4] == ["grid", "grid"]:
                ...
                for c in par_idxs_combinations:
                    ...
                    # Apply reddening + UV-Xrays correlation. Using free parameters the templates must be saved without the effect of that/those parameter/s.
                    bbb_nu0x, bbb_Fnu_redx = BBB_functions[1](
                        bbb_nu, bbb_Fnu_nored, ebvbbb_array[0], 0, gamma[gammai]
                    )
                    # Construct dictionaries
                    BBBFdict_4plot[
                        str(Mbh_array[Mbhi]),
    ...
    ## Model from Done & Kubota. 2018 with spin = 0
        ...
        for c in par_idxs_combinations:
            ...
            model = KD18dict[
                (KD18dict["logBHmass"] == Mbhi) & (KD18dict["logEddra"] == EddRi)
            ]
            bbb_nu, bbb_Fnu_nored = (
                model["wavelength"].values.item(),
                model["SED"].values.item(),
            )
    ...
    ## Model from Done & Kubota. 2018 with spin = 0
        ...
        for c in par_idxs_combinations:
            ...
            model = KD18dict[
                (KD18dict["logBHmass"] == Mbhi)
                & (KD18dict["logEddra"] == EddRi)
                & (KD18dict["warmIndex"] == WarmIndi)
            ]
            bbb_nu, bbb_Fnu_nored = (
                model["wavelength"].values.item(),
                model["SED"].values.item(),
            ...
            if parameters_types[3] == "grid":
                ...
                bbb_nu0, bbb_Fnu_red = BBB_functions[0](bbb_nu, bbb_Fnu_nored, 0)
            # Construct dictionaries
            BBBFdict_4plot[str(Mbhi), str(EddRi), str(WarmIndi), str(ebvi)] = (
                bbb_nu0,
                bbb_Fnu_red,
            )
        ...
        model_functions = [0]
        BBBFdict_4plot = dict()
        THB21dict = pd.read_pickle(
            open(path + "models/BBB/THB21.pickle", "rb")
        )  # THB21_new.pickle
...
def TORUS(path, modelsettings):
    ...
    if modelsettings["TORUS"] == "S04":
        ...
        for nhi in range(nhidx):
            tor_nu0, tor_Fnu0 = (
                S04dict["wavelength"][nhi],
                S04dict["SED"][nhi].squeeze(),
            )
            TORUSFdict_4plot[str(S04dict["Nh-values"][nhi])] = (
                tor_nu0,
                renorm_template("TO", tor_Fnu0),
            )
    ...
    ## Model from Nenkova et al. 2008
        ...
        for incl_i in range(incl_idx):
            tor_nu0, tor_Fnu0 = (
                NK0dict["wavelength"][incl_i],
                NK0dict["SED"][incl_i].squeeze(),
            )
            TORUSFdict_4plot[str(NK0dict["incl-values"][incl_i])] = (
                tor_nu0,
                renorm_template("TO", tor_Fnu0),
            )
    ...
    ## Model from Nenkova et al. 2008
        ...
        NK0_2Pdict = pd.read_pickle(
            open(path + "models/TORUS/NK0_mean_2p.pickle", "rb")
        )
        ...
        for c in par_idxs_combinations:
            ...
            incli = c[1]
            model = NK0_2Pdict[
                (NK0_2Pdict["oa-values"] == oai) & (NK0_2Pdict["incl-values"] == incli)
            ]
            tor_nu0, tor_Fnu0 = (
                model["wavelength"].values.item(),
                model["SED"].values.item(),
            )
            TORUSFdict_4plot[str(oai), str(incli)] = (
                tor_nu0,
                renorm_template("TO", tor_Fnu0),
            )
    ...
    ## Model from Nenkova et al. 2008
        ...
        TORUSFdict_4plot = dict()
        NK0_3Pdict = pd.read_pickle(
            open(path + "models/TORUS/NK0_mean_3p.pickle", "rb")
        )
        ...
        for c in par_idxs_combinations:
            ...
            tvi = c[2]
            model = NK0_3Pdict[
                (NK0_3Pdict["oa-values"] == oai)
                & (NK0_3Pdict["incl-values"] == incli)
                & (NK0_3Pdict["tv-values"] == tvi)
            ]
            tor_nu0, tor_Fnu0 = (
                model["wavelength"].values.item(),
                model["SED"].values.item(),
            )
            TORUSFdict_4plot[str(oai), str(incli), str(tvi)] = (
                tor_nu0,
                renorm_template("TO", tor_Fnu0),
            )
    ...
    ## Model from Stalevski et al. 2016
        ...
        SKIRTORMdict = pd.read_pickle(
            open(path + "models/TORUS/SKIRTOR_mean_1p.pickle", "rb")
        )
        incl_array = SKIRTORMdict["incl-values"]
        # Construct dictionaries
        for incl_i in incl_array:
            tor_nu0, tor_Fnu0 = (
                SKIRTORMdict[SKIRTORMdict["incl-values"] == incl_i]["wavelength"]
                .values.item()
                .to_numpy(),
                SKIRTORMdict[SKIRTORMdict["incl-values"] == incl_i]["SED"]
                .values.item()
                .to_numpy(),
            )
    ...
    ## Model from Stalevski et al. 2016
        ...
        SKIRTORdict = pd.read_pickle(
            open(path + "models/TORUS/SKIRTOR_mean_2p.pickle", "rb")
        )
        ...
        for c in par_idxs_combinations:
            ...
            incli = c[1]
            model = SKIRTORdict[
                (SKIRTORdict["oa-values"] == oai)
                & (SKIRTORdict["incl-values"] == incli)
            ]
            tor_nu0, tor_Fnu0 = (
                model["wavelength"].values.item().to_numpy(),
                model["SED"].values.item().to_numpy(),
            )
            TORUSFdict_4plot[str(oai), str(incli)] = (
                tor_nu0,
                renorm_template("TO", tor_Fnu0),
    ...
    ## Model from Stalevski et al. 2016
        ...
        SKIRTORdict = pd.read_pickle(
            open(path + "models/TORUS/SKIRTOR_mean_3p.pickle", "rb")
        )
    ...
    ## Model from Stalevski et al. 2016
        ...
        CAT3Ddict = pd.read_pickle(
            open(path + "models/TORUS/CAT3D_mean_3p.pickle", "rb")
        )

        incl_array = CAT3Ddict["incl-values"].unique()
        a_array = CAT3Ddict["a-values"][
            210:
        ].unique()  # Value of the 2nd set of 168 SEDs
        ...
        for c in par_idxs_combinations:
            ...
            fwdi = c[2]
            model = CAT3Ddict[
                (CAT3Ddict["incl-values"] == incli)
                & (CAT3Ddict["a-values"] == ai)
                & (CAT3Ddict["fwd-values"] == fwdi)
            ]
            tor_nu0, tor_Fnu0 = (
                model["wavelength"].values.item(),
                model["SED"].values.item(),
            )
            TORUSFdict_4plot[str(incli), str(ai), str(fwdi)] = (
                tor_nu0,
                renorm_template("TO", tor_Fnu0),
            )
...
def XRAYS(
    bbb_nu, bbb_Fnu, scatter, Gamma=1.8
):  # Use the UV-Xrays correlation (Just et a. 2007) to extend SEDs, assuming a power law emission in Xrays
    # Interpolation to the nearest neighbors
    f = interp1d(bbb_nu, bbb_Fnu, kind="nearest", bounds_error=False, fill_value=0.0)
    ...
    a = Fnu_2kev / (
        (h * nu_2kev) ** (-Gamma + 1) * np.e ** (-nu_2kev / (7.2540 * 1e19))
    )

    xray_nu = np.logspace(
        16.685, 19.7, 1000
    )  # with a hole between BB template and X-Rays
...
def BBBred_Prevot(bbb_x, bbb_y, BBebv):
    ...
    w1 = tuple(
        [redd_x <= wl_200eV]
    )  # wavelenghts correspond to energies higher than UV

    #    Define prevots function for the reddening law redd_k
    def function_prevot(x, RV):
        y = 1.39 * pow((pow(10.0, -4.0) * x), -1.2) - 0.38
        return y

    bbb_k[w0] = function_prevot(redd_x[w0], RV)
    bbb_k[w1] = (
        0  # If wavelenghts correspond to energies higher than UV, there is no reddening
    )
    bbb_k = bbb_k[::-1]
...
def GALAXYred_Calzetti(gal_nu, gal_Fnu, GAebv):
    ...
    k[w1] = (
        2.659
        * (-2.156 + (1.509 / wl[w1]) - (0.198 / wl[w1] ** 2) + (0.011 / wl[w1] ** 3))
        + RV
    )
...
def fluxlambda_2_fluxnu(flux_lambda, wl_angst):
    ...
    flux_nu = flux_lambda * (wl_angst**2.0) / c / Angstrom
    return flux_nu
...
def stellar_info(chain, data, models, mc):
    ...
    if mc["sampling_algorithm"] == "ultranest":
        if models.settings["RADIO"] == False and (
            models.settings["BBB"] == "R06" or models.settings["BBB"] == "THB21"
        ):
            GA = chain.iloc[:, -4]
        elif models.settings["RADIO"] == False and (
            models.settings["BBB"] != "R06" and models.settings["BBB"] != "THB21"
        ):
            GA = chain.iloc[:, -3]
        elif models.settings["RADIO"] == True and (
            models.settings["BBB"] == "R06" or models.settings["BBB"] == "THB21"
        ):
            GA = chain.iloc[:, -5]
        elif models.settings["RADIO"] == True and (
            ...
            GA = chain.iloc[
                :, -4
            ]  # - 18. ## 1e18 is the common normalization factor used in parspace.ymodel
    ...
    if mc["sampling_algorithm"] == "ultranest":
        for i in range(len(tau_mcmc)):
            ...
            if len(gal_obj.par_names) == 3:
                gal_obj.pick_nD(tuple([tau_mcmc.iloc[i], age_mcmc.iloc[i], 0.0]))
                tau_dct, age_dct, ebvg_dct = gal_obj.matched_parkeys
                SFR_mcmc = MD.GALAXY_SFRdict[tau_dct, age_dct]
            elif len(gal_obj.par_names) == 4:
                gal_obj.pick_nD(
                    tuple([metal_mcmc.iloc[i], tau_mcmc.iloc[i], age_mcmc.iloc[i], 0.0])
                )
        ...
        for i in range(len(tau_mcmc)):
            ...
            if len(gal_obj.par_names) == 3:
                gal_obj.pick_nD(tuple([tau_mcmc[i], age_mcmc[i], 0.0]))
                tau_dct, age_dct, ebvg_dct = gal_obj.matched_parkeys
                SFR_mcmc = MD.GALAXY_SFRdict[tau_dct, age_dct]
            elif len(gal_obj.par_names) == 4:
                gal_obj.pick_nD(tuple([metal_mcmc[i], tau_mcmc[i], age_mcmc[i], 0.0]))
                metal_dct, tau_dct, age_dct, ebvg_dct = gal_obj.matched_parkeys
                SFR_mcmc = MD.GALAXY_SFRdict[metal_dct, tau_dct, age_dct]
...
def stellar_info_array(chain_flat, data, models, Nthin_compute, mc):
    ...
    if mc["sampling_algorithm"] == "ultranest":
        chain_thinned = chain_flat.iloc[
            random.sample(list(np.arange(Ns)), Nthin_compute)
        ]
