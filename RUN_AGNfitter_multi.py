...
def MAKE_model_dictionary(
    cat_settings, filters_settings, models_settings, clobbermodel=False
):
    """
    ...
    modelsdict_name = (
        cat_settings["path"]
        + models_settings["path"]
        + filters_settings["filterset"]
        + models_settings["modelset"]
    )

    if clobbermodel and os.path.lexists(
        modelsdict_name
    ):  ## If overwriting of model dictionary mode is on
    ...
    if not os.path.lexists(modelsdict_name):  ## If model dictionary does not exist yet
        ...
        mydict = MODELSDICT(
            modelsdict_name, cat_settings["path"], filters_settings, models_settings
        )
        mydict.build()
        ...
        print("________________________")
        print(
            "The models dictionary " + modelsdict_name + " has been created."
            "(%.2g min elapsed)" % ((time.time() - t0) / 60.0)
        )
        ...
        test_settingschanges = [
            mydict.filters_list[i] == filters_settings[i]
            for i in filters_settings.keys()
            if type(filters_settings[i]) is BooleanType
        ]

        if (
            False in test_settingschanges
        ):  ##  compare nr of data bands with n of model filters
            print("________________________")
            print("*** WARNING ***")
            print(
                "You have changed entries in your FILTER_settings without updating the filterset name."
            )
            ...
            if py3:
                response = input(
                    ">> You want to continue with the old model dictionary nevertheless? (yes/no)"
                )
            else:
                response = raw_input(
                    ">> You want to continue with the old model dictionary nevertheless? (yes/no)"
    ...
    Modelsdict = (
        mydict.MD
    )  ## MD is the model dictionary saved as a class of the method MODELSDICT

    bands_in_cat = len(cat_settings["freq/wl_list"])
    bands_in_dict = len(
        Modelsdict[Modelsdict.keys()[0]][1][
            Modelsdict[Modelsdict.keys()[0]][1].keys()[0]
        ][0]
    )
    ...
    if bands_in_cat != bands_in_dict:
        sys.exit(
            ...
            + "Number of bands in catalog ("
            + str(bands_in_cat)
            + "), does not match the numbers of filters in models ("
            + str(bands_in_dict)
            + ")\n"
...
def RUN_AGNfitter_onesource_independent(
    line, data_obj, filtersz, models_settings, mc_settings, clobbermodel=False
):
    """
    ...
    dictz = (
        cat_settings["output_folder"] + str(data.name) + "/MODELSDICT_" + str(data.name)
    )
    ...
    if os.path.lexists(
        cat_settings["output_folder"] + str(data.name) + "/samples_mcmc.sav"
    ) or os.path.lexists(
        cat_settings["output_folder"]
        + str(data.name)
        + "/ultranest/chains/weighted_post.txt"
    ):
        print("Done")
        ...
        try:
            if not os.path.lexists(dictz):
                ...
                print(
                    "For this dictionary creation %.2g min elapsed"
                    % ((time.time() - t0) / 60.0)
                )
            else:
                dictz = str.encode(dictz)
            ...
            P = parspace.Pdict(
                data, models
            )  # Dictionary with all parameter space specifications.
            # From PARAMETERSPACE_AGNfitter.py

            t1 = time.time()

            try:
                PLOTandWRITE_AGNfitter.main(
                    data, models, P, out, models_settings, mc_settings
                )
                print("Done already")
            except:
                print("Not done yet")
            ...
            print("_____________________________________________________")
            print("For this fit %.2g min elapsed" % ((time.time() - t1) / 60.0))
...
def RUN_AGNfitter_multiprocessing(
    processors, data_obj, models_settings, mc_settings, indep_bool=False, filters="None"
):
    """
    ...
    if indep_bool == False:
        ...
        print(
            "processing all {0:d} sources with {1:d} cpus".format(nsources, processors)
        )

        pool = mp.Pool(processes=processors)
        catalog_fitting = pool.map(
            multi_run_wrapper,
            zip(
                range(nsources),
                itertools.repeat(data_obj),
                itertools.repeat(models_settings),
                itertools.repeat(mc_settings),
            ),
        )
        pool.close()
        pool.join()
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        type=int,
        default=-1,
        help="specify a single source number to run (this is the line number in hte catalogue not the source id/name)",
    )
    parser.add_argument(
        "-i",
        "--independent",
        action="store_true",
        help="run independently per source, i.e. do not create a global model dictionary",
    )
    parser.add_argument(
        "-o", "--overwrite", action="store_true", help="overwrite model files"
    )
    ...
    if args.independent:
        if args.ncpu > 1.0:
            ...
            RUN_AGNfitter_onesource_independent(
                args.sourcenumber,
                data_ALL,
                filters_settings,
                models_settings,
        ...
        print(
            'Option of one single dictionary for a whole catalogue is deprecated. Running "independent, -i" option.'
        )
